...
void task_cmd(void * pvParameters)
{
	...
	{	
		...
		if(xSemaphoreTake(xUartSemaphore, portMAX_DELAY) == pdPASS)	
		{
			...
			{
				//若舵机正在后退，挂起舵机后退任务
				if(status[6] == 4 && task_back_Handle != NULL)
				{
			...
			{
				//若舵机正在前进，挂起舵机前进任务
				if(status[6] == 3 && task_advance_Handle != NULL)
				{
			...
			else if(strcmp(cmd_receive, "GET TEM") == 0)//读取温度
			{
				...
				if(status[7] == 2 && task_get_hum_Handle != NULL)
				{
					vTaskSuspend(task_get_hum_Handle);
				}
				if(status[7] == 3 && task_get_tem_hum_Handle != NULL)
				{
					vTaskSuspend(task_get_tem_hum_Handle);
				}
				//任务存在，解挂任务，任务不存在，创建任务
				...
				{
					status[7] = 1;
					vTaskResume(task_get_tem_Handle);
				}
			}
			else if(strcmp(cmd_receive, "GET HUM") == 0)//读取湿度
			{
				//如果没有在读取湿度，则挂起其他在用sht20的任务
				if(status[7] == 1 && task_get_tem_Handle != NULL)
				{
					vTaskSuspend(task_get_tem_Handle);
				}
				if(status[7] == 3 && task_get_tem_hum_Handle != NULL)
				{
					vTaskSuspend(task_get_tem_hum_Handle);
				...
				{
					status[7] = 2;
					vTaskResume(task_get_hum_Handle);
				}
			}
			else if(strcmp(cmd_receive, "GET TEM_HUM") == 0)//读取温湿度
			{
				...
				{
					status[7] = 3;
					vTaskResume(task_get_tem_hum_Handle);
				}
			}
			...
			else if(strcmp(cmd_receive, "FAN LOW") == 0)//风扇,低档
			{
