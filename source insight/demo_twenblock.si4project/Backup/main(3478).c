...
void task_cmd_read( void * pvParameters )
{
	for(;;)
	{
		if(cmd_ready==1)
		{
			...
			if(strcmp(cmd_read,"LEDD2 OPEN") == 0)
			{
	...
	for(;;)
	{
		if(cmd_ready == 0 && USART_GetFlagStatus(USART2, USART_FLAG_RXNE) == SET)
		{
			...
			else
			{
				...
				if(strcmp(cmd_write,"SERVO ADVANCE") == 0)
				{
				...
				else if(strcmp(cmd_write,"SERVO BACK") == 0)
				{
				...
				else if(strcmp(cmd_write,"LED RUN") == 0)
				{
