...
void USART2_IRQHandler(void)
{
	...
	uint32_t ulReturn;
	BaseType_t xResumeWoken = pdFALSE; // 存储xTaskResumeFromISR的返回值
	if(USART_GetITStatus(USART2, USART_IT_RXNE))
	{
		...
		portYIELD_FROM_ISR(xTaskResumeFromISR(task_cmd_Handle));
	}
	portYIELD_FROM_ISR(xHigherPriorityTaskWoken || xResumeWoken);
}
