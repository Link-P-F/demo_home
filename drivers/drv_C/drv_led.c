...
void drv_led_open(uint32_t led_id)
{
	...
	{
		case LED_ID_1:
			GPIO_SetBits(LED_PORT_123, LED_PIN_1);
			break;
		case LED_ID_2:
			GPIO_SetBits(LED_PORT_123, LED_PIN_2);
			break;
		case LED_ID_3:
			GPIO_SetBits(LED_PORT_123, LED_PIN_3);
			break;
		case LED_ID_D2:
...
其他说明:LED引脚、所属组见"drv_led.h"
		led1、led2、led3高电平亮，ledD2低电平亮
***************************************************/
void drv_led_close(uint32_t led_id)
{
	switch (led_id)
	{
		case LED_ID_1:
...
/***************************************************
函数名称：drv_led_run
函数功能：led1\led2\led3轮流开关
输入参数：无
输出参数：无
其他说明：延时函数使用freertos所自带的
***************************************************/
extern int status[8];
